...
class VoiceSearchUiBottomSheet: UIViewController ,StreamingDelegate{
    ...
    public func onResult(data: String) {
        DispatchQueue.main.async { [self] in
            ...
            // output=data
            if let str = convertToDictionary(text: data), let displayText = str["display_text"] as? String {
                self.outputLabel.text=displayText
            }
            
            if let str = convertToDictionary(text: data), let displayText = str["display_text"] as? String, let final = str["final"] as? Bool, final {
                if(final)
